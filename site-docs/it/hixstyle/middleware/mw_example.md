...
Questo capitolo finale mostra un esempio pratico di come potrebbe apparire una semplice applicazione,
nella quale avremo una schermata di autenticazione (login), un CRUD, e 3 moduli finti.

Per lo schema di questa applicazione le "mie condizioni" (perché ognuno ha le proprie)
sono:
...
Possiamo osservare come uso 3 tipi di Mw. Una struttura che valida semplicemente se sei
autenticato (MyAppAuth) a se sono validato e ho un ruolo specifico (MyAppRole)
