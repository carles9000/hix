#!/usr/bin/env bash
# --------------------------------------------------------------
# go_gcc.sh - Build and run HIX example server on Linux/gcc
#
# Linux equivalent of go_msvc64.bat / go_mingw64.bat. Compiles
# examples/server/hix.hbp against libhix_server.a and (if compile
# succeeds) executes the resulting binary.
#
# Usage:
#   ./go_gcc.sh                          build + run server
...
export HB_ROOT   # needed if hix.hbp references ${HB_ROOT}/... paths
...
# ${hix} in hix.hbp expands from this env var (-L${hix}, ${hix}/hix_server.hbx).
export hix="$(cd ../.. && pwd)"
