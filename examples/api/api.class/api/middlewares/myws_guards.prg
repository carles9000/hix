/*-----------------------------------------------------------
  ...
  Description: Consolidated custom middleware for Fenix.WS.Lite.
               ...
               Public middlewares:
                 ...
                 MyWsEnforceJson    -- 415 if POST/PUT/PATCH not JSON
...
// ============================================================
// MyWsEnforceJson -- reject POST/PUT/PATCH without JSON body.
// GET/DELETE/HEAD/OPTIONS pass through untouched.
// ============================================================
FUNCTION MyWsEnforceJson( oCtx )
   ...
   IF cMethod == "GET" .OR. cMethod == "DELETE" .OR. ;
      cMethod == "HEAD" .OR. cMethod == "OPTIONS"
      RETURN .T.
   ENDIF

   cCt := Lower( UHeader( "Content-Type", "" ) )

   IF Empty( cCt )
      USendApiError( "MISSING_CONTENT_TYPE", ;
         "Content-Type header is required", 415, ;
         { "hint" => "Send Content-Type: application/json" } )
      oCtx:lHandled := .T.
      RETURN .F.
   ENDIF

   IF !( "application/json" $ cCt )
      USendApiError( "UNSUPPORTED_MEDIA_TYPE", ;
         "Only application/json is accepted", 415, ;
...
FUNCTION MyWsGuardPublic( oCtx )
   ...
   IF ! MyWsEnforceJson  ( oCtx ) ; RETURN .F. ; ENDIF
