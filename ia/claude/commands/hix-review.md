...
- **Token 1** -> project path (optional, default: cwd). Must contain
  `hix.exe`, `hix.json`, and `www/` for the agent to proceed. If missing,
  the agent will abort with a pointer to `/hix-init`.

## What the agent does

Reads the project files under `www/`, walks a checklist of public HIX /
Harbour rules (LOCAL placement, `!=` on strings, `hb_UnixTime`, action
string format, `USendView` positional args, MW registration via loaders,
whitelist ACL, etc.), and produces a findings report classified as
blocker / warning / suggestion. Read-only: it never edits, builds, or
fixes. See `~/.claude/agents/hix-reviewer.md` for the full contract.
